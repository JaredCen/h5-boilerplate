...
export function on(type, selector, target, fn) {
    if (typeof selector === 'function') {
        event.on(type, selector);
    } else {
        ...
        event.add(CONTEXT, _type, _fn);
    }
};

export function one(type, selector, target, fn) {
    let _fn = () => {
        fn.apply(this, arguments);
...
export function off(type, selector, fn) {
    if (typeof selector === 'function' || typeof selector === 'undefined') {
        event.off(type, selector);
    } else {
        const _selector = _getElement(selector);
        if (!_selector) {
